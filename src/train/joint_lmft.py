...
def get_model(config:dict) -> nn.Module:
    ...
    model.load_state_dict(torch.load(opj('../res', config.args.evaluation_id, f'{best_epoch}epoch', 'model.state')), strict=True)

    ##____ large margin finetuning: update model margine and topK
    model.aam_softmax.m = config.model.margin
    model.aam_softmax.update_margins(config.model.margin, config.model.margin_penalty, config.model.topK)
...
def train_model(config:dict, logger:Union[neptune.Run, None]):
    ...
    printlog("\n=========================== PHASE 3: TRAIN_LARGE_MARGIN_FINETUNE ==========================", logfile)
