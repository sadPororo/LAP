...
def random_state_init(seed:int=42):
    ...
    ##____GPU environ
    # torch.cuda.device(gpu_id).manual_seed(seed) 
...
def get_random_state(device_list:list):
    ...
    cuda_state = {}
    for rank, gpu_id in enumerate(device_list):
        cuda_state[rank] = torch.cuda.get_rng_state(device=int(gpu_id))
...
def hypwrite(config:EasyDict, yaml_path:str):
    """ Write '.yaml' file
    ...
    """
    ##____ EasyDict object to dictionary class
    config = {k: dict(v) if type(v)==EasyDict else v for (k,v) in config.items()}
...
def ddp_setup(is_master:bool):
    """ Disable print() when the process is not master """
